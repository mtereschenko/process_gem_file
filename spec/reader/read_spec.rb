  ...
  describe '.parse_groups' do
    context 'should parse groups of gems of provided Gemfile' do
      it 'and return hash with parsed groups' do
        expected_hash = {
          'default' => %w[puma rails sqlite3 tzinfo-data],
          ':development' => %w[bullet web-console listen rack-mini-profiler faker spring],
          ':development, :test' => %w[byebug rspec-rails],
          ':test' => %w[capybara selenium-webdriver]
        }
        reader = Reader::Read.new(@source_file_name)
